...
const setup = async ({
  ...
  member = null,
  showPopup = true,
  giftResponse = defaultGiftResponse,
  giftError = null,
  offer = null,
}) => {
...
describe('Portal Data links:', () => {
    ...
    describe('on a paid-members only site', () => {
      describe('with only a free plan', () => {
        test('renders invite-only message and does not allow signups', async () => {
          ...
          const inviteOnlyMessage = within(popupFrame.contentDocument).queryByText(
            /This site is invite-only/i,
          );
      ...
      describe('with paid plans', () => {
        test('allows paid signups', async () => {
          ...
          const chooseBtns = within(popupFrame.contentDocument).queryAllByRole('button', {
            name: 'Choose',
          });
  ...
  describe('#/portal/signup/free', () => {
    ...
    describe('on a paid-members only site', () => {
      test('renders paid-members only message and does not allow signups', async () => {
        ...
        const paidMembersOnlyMessage = within(popupFrame.contentDocument).queryByText(
          /This site only accepts paid members/i,
        );
  ...
  describe('#/portal/account/newsletter/help', () => {
    test('opens portal newsletter receiving help page', async () => {
      ...
      popupFrame = await utils.findByTitle(/portal-popup/i);
      expect(popupFrame).toBeInTheDocument();
      const helpPageTitle = within(popupFrame.contentDocument).queryByText(
        /help! i'm not receiving emails/i,
      );
  ...
  describe('#/portal/account/newsletter/disabled', () => {
    test('opens portal newsletter receiving help page', async () => {
      ...
      expect(popupFrame).toBeInTheDocument();
      const helpPageTitle = within(popupFrame.contentDocument).queryByText(
        /why has my email been disabled/i,
      );
      expect(helpPageTitle).toBeInTheDocument();
    });
  });
  ...
  describe('#/portal/gift', () => {
    test('opens gift page', async () => {
      ...
      const giftSubtitle = within(popupFrame.contentDocument).queryByText(
        /share a full membership.*with a friend or colleague/i,
      );
  ...
  describe('#/portal/gift/redeem/<token>', () => {
    ...
    const setupGiftRedemption = async ({
      giftError = null,
      giftResponse = defaultGiftResponse,
    } = {}) => {
    ...
    const expectGiftRedemptionErrorToast = async ({ utils, subtitle }) => {
      ...
      expect(
        await within(notificationIframeDocument).findByText(/Gift could not be redeemed/i),
      ).toBeInTheDocument();
    ...
    test('renders gift redemption popup without name/email inputs for a logged-in free member', async () => {
      let { ghostApi, popupFrame, triggerButtonFrame, ...utils } = await setupGiftRedemption();
  ...
  describe('unauthenticated account page access', () => {
    test.each([
      ...
      '#/portal/$label redirects to signin with redirect URL when not logged in',
      async ({ path }) => {
  ...
  describe('hashchange account page access', () => {
    test.each([
      ...
      '#/portal/$path opens account page via hashchange when logged in',
    ...
    test.each([
      ...
      '#/portal/$label redirects to signin via hashchange when not logged in',
      async ({ path }) => {
        // Start with no hash — simulates an already-loaded page
        window.location.hash = '';
        let { ghostApi, popupFrame, triggerButtonFrame, ...utils } = await setup({
