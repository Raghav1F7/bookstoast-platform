...
const IntegrationItem: React.FC<IntegrationItemProps> = ({
  ...
  const buttons = custom ? (
    ...
    <Button size="sm" type="button" variant="ghost" onClick={handleClick}>
      <LucideIcon.Lock />
      Upgrade
    </Button>
  ) : (
  ...
  return (
    <ActionListItem className={disabled ? 'opacity-50 saturate-0' : ''} data-testid={testId}>
        ...
        >
          ...
          <span className="min-w-0 grow">
            <span className="flex items-center gap-1">
              {title}
              {active && (
                <span className="inline-flex items-center rounded-full bg-green/10 px-1.5 py-px text-xs font-semibold tracking-wide text-green uppercase">
                  Active
                </span>
              )}
            </span>
      ...
      <ActionListItemActions visibility={disabled ? 'always' : 'hover'}>
        {buttons}
      </ActionListItemActions>
    </ActionListItem>
  );
          ...
          <IntegrationItem
            ...
            detail={
              <div className="line-clamp-2 break-words">
                <span title={`${integration.name}: ${integration.description || 'No description'}`}>
